 ...
 * Fully-blocked roads encountered along the way are also cleared. If there is
 * no target then this agent does nothing.
 */
public class ControlledPoliceForce extends StandardAgent<PoliceForce> {
  ...
  public String toString() {
    ...
    return "Human controlled police force " + getID()
        + ( target == null ? " (no target)"
            : " target: road " + target.getID() + " with "
                + ( target.isBlockadesDefined() ? " unknown"
                    : String.valueOf( target.getBlockades().size() ) )
                + " blockades" );
  }
