...
/**
 * A basic fire brigade agent that will try to extinguish a given target. If the
 * target is a refuge then the fire brigade will attempt to enter the building
 * to replenish water. If there is no target then this agent does nothing.
 */
public class ControlledFireBrigade extends StandardAgent<FireBrigade> {
  ...
  public String toString() {
    ...
    return "Human controlled fire brigade " + me().getID() + " ("
        + me().getWater() + " water)" + ( target == null ? " (no target)"
            : " target: building " + target.getID() );
  }
}