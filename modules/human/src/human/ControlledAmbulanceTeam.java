...
/**
 * A basic ambulance team agent that will try to rescue a given target. Once the
 * target is unburied this agent will attempt to load it and transport it to a
 * refuge. If there is no target then this agent does nothing.
 */
      ...
      if ( target.getPosition().equals( getID() ) ) {
        ...
        if ( location() instanceof Refuge ) {
          ...
          List<EntityID> path = search.breadthFirstSearch( me().getPosition(),
              objectsToIDs(
                  model.getEntitiesOfType( StandardEntityURN.REFUGE ) ) );
          if ( path != null ) {
            sendMove( time, path );
            return;
          } else {
