  ...
  private void updateRefuges() {
    ...
    for (StandardEntity e : model.getEntitiesOfType(StandardEntityURN.REFUGE)) {
      if (e instanceof Refuge) {
        Refuge refuge = (Refuge) e;

        // System.out.println("Refuge = " + e.getID() + " bed cap = " +
        // refuge.getBedCapacity() + " occupied beds "
        // + refuge.getOccupiedBeds() + " beds size = " +
        // this.beds.get(e.getID()).size() + " wait size = "
        // + this.waitingList.get(e.getID()).size());

        while (refuge.getOccupiedBeds() < refuge.getBedCapacity()) {
          if (this.waitingList.get(refuge.getID()).size() > 0) {
            this.beds.get(refuge.getID()).add(this.waitingList.get(refuge.getID()).remove());
