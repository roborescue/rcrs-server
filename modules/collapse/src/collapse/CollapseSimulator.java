...
public class CollapseSimulator extends StandardSimulator implements GUIComponent {
  ...
  protected void postConnect() {
    super.postConnect();
    this.stats = new EnumMap<StandardEntityConstants.BuildingCode, CollapseStats>(
    ...
    for (StandardEntityConstants.BuildingCode code : StandardEntityConstants.BuildingCode.values()) {
      this.stats.put(code, new CollapseStats(code, config));
    }
    slight = new GaussianGenerator(this.config.getFloatValue(CONFIG_PREFIX + SLIGHT_MEAN_SUFFIX),
        this.config.getFloatValue(CONFIG_PREFIX + SLIGHT_SD_SUFFIX), this.config.getRandom());
    moderate = new GaussianGenerator(this.config.getFloatValue(CONFIG_PREFIX + MODERATE_MEAN_SUFFIX),
        this.config.getFloatValue(CONFIG_PREFIX + MODERATE_SD_SUFFIX), this.config.getRandom());
    severe = new GaussianGenerator(this.config.getFloatValue(CONFIG_PREFIX + SEVERE_MEAN_SUFFIX),
        this.config.getFloatValue(CONFIG_PREFIX + SEVERE_SD_SUFFIX), this.config.getRandom());
    destroyed = new GaussianGenerator(this.config.getFloatValue(CONFIG_PREFIX + DESTROYED_MEAN_SUFFIX),
        this.config.getFloatValue(CONFIG_PREFIX + DESTROYED_SD_SUFFIX), this.config.getRandom());
    this.block = this.config.getBooleanValue(BLOCK_KEY);
    this.floorHeight = this.config.getFloatValue(FLOOR_HEIGHT_KEY) * 1000;
    this.extent = new ContinuousUniformGenerator(this.config.getFloatValue(WALL_COLLAPSE_EXTENT_MIN_KEY),
        this.config.getFloatValue(WALL_COLLAPSE_EXTENT_MAX_KEY), this.config.getRandom());
    this.buildingCache = new HashSet<Building>();
    this.roadCache = new HashSet<Road>();
    for (StandardEntity next : model) {
      if ((next instanceof Building) && (!(next instanceof Refuge))) {
    ...
    this.model.addWorldModelListener(new WorldModelListener<StandardEntity>() {
  ...
  private Collection<Building> doCollapse(ChangeSet changes, int time) {
    ...
    if (gui != null) {
      gui.startCollapse(this.buildingCache.size());
    }
    if (time == 1) {
    ...
    if (gui != null) {
      gui.startFire(this.buildingCache.size());
    }
  ...
  private Collection<Building> doEarthquakeCollapse(ChangeSet changes) {
    ...
    for (Building b : this.buildingCache) {
      ...
      /*
       * Aftershock Requirement 2013 This ignores the new damage of it is less than
       * the previous
       */
  ...
  private Collection<Building> doFireCollapse(ChangeSet changes) {
    ...
    for (Building b : buildingCache) {
      /*
       * Aftershock Requirement @ 2103: it is used in doBlock to check whether apply
       * the aftershock intensity or not
       */
  ...
  private void createBlockages(Building b, Map<Road, Collection<java.awt.geom.Area>> roadBlockages, int time) {
    ...
    /*
     * Aftershock's Requirement@2103: this checks the ratio of the building's floors
     * that is possible to collapsed
     */
    ...
    /*
     * Aftershock Requirement 2013: applies the intensity of the aftershock. This
     * only applies if there is an aftershock and time is >1 and the building
     * doesn't have any fire damage. In case of any fire damage the building creates
     * the max possible collapse.
     */
  ...
  private void projectWall(Edge edge, Collection<java.awt.geom.Area> areaList, double d) {
    Line2D wallLine = new Line2D(edge.getStartX(), edge.getStartY(), edge.getEndX() - edge.getStartX(),
    ...
    Ellipse2D ellipse1 = new Ellipse2D.Double(right.getX() - radius, right.getY() - radius, radius * 2, radius * 2);
    Ellipse2D ellipse2 = new Ellipse2D.Double(left.getX() - radius, left.getY() - radius, radius * 2, radius * 2);
    areaList.add(new java.awt.geom.Area(ellipse1));
    areaList.add(new java.awt.geom.Area(ellipse2));
  }

  private Map<Road, Collection<java.awt.geom.Area>> createRoadBlockades(java.awt.geom.Area buildingArea,
      java.awt.geom.Area existing) {
    ...
    Collection<StandardEntity> roads = model.getObjectsInRectangle((int) rectangle.getMinX(), (int) rectangle.getMinY(),
  ...
  private class CollapseStats {
    ...
    CollapseStats(StandardEntityConstants.BuildingCode code, Config config) {
      ...
      this.pDestroyed = config.getFloatValue(s + DESTROYED_SUFFIX);
      this.pSevere = this.pDestroyed + config.getFloatValue(s + SEVERE_SUFFIX);
      this.pModerate = this.pSevere + config.getFloatValue(s + MODERATE_SUFFIX);
      this.pSlight = this.pModerate + config.getFloatValue(s + SLIGHT_SUFFIX);
    }
  ...
  public CollapseWorldModel model() {
    return (CollapseWorldModel) this.model;
  }

  @Override
  protected void processMessage(Message msg) {
    if (msg instanceof KSAfterShockInfo) {
      model().updateAftershocks((KSAfterShockInfo) msg);
    } else {
