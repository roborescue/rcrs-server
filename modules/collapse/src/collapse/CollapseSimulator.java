...
public class CollapseSimulator extends StandardSimulator
  ...
  protected void postConnect() {
    ...
    slight = new GaussianGenerator(
        config.getFloatValue( CONFIG_PREFIX + SLIGHT_MEAN_SUFFIX ),
        config.getFloatValue( CONFIG_PREFIX + SLIGHT_SD_SUFFIX ),
        config.getRandom() );
    moderate = new GaussianGenerator(
        config.getFloatValue( CONFIG_PREFIX + MODERATE_MEAN_SUFFIX ),
        config.getFloatValue( CONFIG_PREFIX + MODERATE_SD_SUFFIX ),
        config.getRandom() );
    severe = new GaussianGenerator(
        config.getFloatValue( CONFIG_PREFIX + SEVERE_MEAN_SUFFIX ),
        config.getFloatValue( CONFIG_PREFIX + SEVERE_SD_SUFFIX ),
        config.getRandom() );
    destroyed = new GaussianGenerator(
        config.getFloatValue( CONFIG_PREFIX + DESTROYED_MEAN_SUFFIX ),
  ...
  protected void processCommands( KSCommands c, ChangeSet changes ) {
    ...
    Map<Road, Collection<Blockade>> blockades = createBlockadeObjects(
        newBlock );
  ...
  private Map<Road, Collection<java.awt.geom.Area>>
      doBlock( Collection<Building> collapsed, int time ) {
  ...
  private Collection<Building> doEarthquakeCollapse( ChangeSet changes ) {
    ...
    for ( Building b : buildingCache ) {
      ...
      /*
       * Aftershock Requirement 2013: this ignores the new damage of it is less
       * than the previous
       */
    ...
    for ( StandardEntityConstants.BuildingCode code : StandardEntityConstants.BuildingCode
        .values() ) {
      Logger.info(
          "Building code " + code + ": " + total.get( code ) + " buildings" );
  ...
  private Collection<Building> doFireCollapse( ChangeSet changes ) {
    ...
    for ( Building b : buildingCache ) {
      /*
       * Aftershock Requirement @ 2103: it is used in doBlock to check whether
       * apply the aftershock intensity or not
       */
  ...
  private void createBlockages( Building b,
    ...
    /*
     * Aftershock's Requirement@2103: this checks the ratio of the building's
     * floors that is possible to collapsed
     */
    ...
    /* Aftershock's Requirement: 2013 */
    model().getCSBuiding( b ).increaseCollapseRatio( (float) ( d
        / model().getCSBuiding( b ).getTotalCollapse( floorHeight ) ) );
  ...
  private void projectWall( Edge edge, Collection<java.awt.geom.Area> areaList,
      double d ) {
  ...
  private java.awt.geom.Area
      areaToGeomArea( rescuecore2.standard.entities.Area area ) {
