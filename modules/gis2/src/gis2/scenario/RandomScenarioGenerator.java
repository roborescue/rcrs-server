...
public class RandomScenarioGenerator {
  ...
  private static final int DEFAULT_MIN_BEDCAPACITY = 0;
  private static final int DEFAULT_MAX_BEDCAPACITY = 10;
  private static final int DEFAULT_MIN_REFILLCAPACITY = 0;
  private static final int DEFAULT_MAX_REFILLCAPACITY = 1000;
  ...
  private int minBedCapacity;
  private int maxBedCapacity;
  private int minRefillCapacity;
  private int maxRefillCapacity;
  ...
  public RandomScenarioGenerator() {
    ...
    minBedCapacity = DEFAULT_MIN_BEDCAPACITY;
    maxBedCapacity = DEFAULT_MAX_BEDCAPACITY;
    minRefillCapacity = DEFAULT_MIN_REFILLCAPACITY;
    maxRefillCapacity = DEFAULT_MAX_REFILLCAPACITY;
  }
  ...
  public static void main(String[] args) {
    ...
    for (int i = 1; i < args.length; ++i) {
      ...
      } else if ("-beds".equals(args[i])) {
        int min = Integer.parseInt(args[i + 1]);
        int max = Integer.parseInt(args[i + 2]);
        i += 2;
        generator.setBedCapacity(min, max);
      } else if ("-refill".equals(args[i])) {
        int min = Integer.parseInt(args[i + 1]);
        int max = Integer.parseInt(args[i + 2]);
        i += 2;
        generator.setRefillCapacity(min, max);
  ...
  private static void printUsage() {
    ...
    System.out.println("-beds\tmin max\tSet the minimum and maximum bed capacity of refuges");
    System.out.println("-refill\tmin max\tSet the minimum and maximum refill capacity of refuges");
  ...
  /**
   * Set the minimum and maximum bed capacity of refuges.
   *
   * @param min The new minimum.
   * @param max The new maximum.
   */
  public void setBedCapacity(int min, int max) {
    minBedCapacity = min;
    maxBedCapacity = max;
  }

  /**
   * Set the minimum and maximum refill capacity of refuges.
   *
   * @param min The new minimum.
   * @param max The new maximum.
   */
  public void setRefillCapacity(int min, int max) {
    minRefillCapacity = min;
    maxRefillCapacity = max;
  }
  ...
  public GisScenario makeRandomScenario(GMLMap map, Random random) {
    ...
    placeRefuges(it, result, random, refuge);
  ...
  private void placeRefuges(Iterator<GMLBuilding> it, GisScenario result, Random random, int num) {
    for (int i = 0; i < num; ++i) {
      int bedCapacity = random.nextInt(maxBedCapacity - minBedCapacity + 1) + minBedCapacity;
      int refillCapacity = random.nextInt(maxRefillCapacity - minRefillCapacity + 1) + minRefillCapacity;

      result.addRefuge(it.next().getID(), bedCapacity, refillCapacity);
    }
  }
