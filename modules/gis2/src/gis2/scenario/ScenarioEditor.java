...
public class ScenarioEditor extends JPanel {
  ...
  private static final Color AMBULANCE_CENTRE_COLOUR = new Color( 255, 255,
      255 );
  ...
  private FilledShapeDecorator hydrantDecorator = new FilledShapeDecorator(
      null, HYDRANT_COLOUR, null );
  ...
  public ScenarioEditor( JMenuBar menuBar, GMLMap map, GisScenario scenario ) {
    ...
    inspector.setPreferredSize( new Dimension( INSPECTOR_PREFERRED_WIDTH,
        INSPECTOR_PREFERRED_HEIGHT ) );
  ...
  public void updateGMLRefuges() {
    for ( int next : scenario.getRefuges() ) {
      GMLRefuge refuge = new GMLRefuge( next,
          map.getBuilding( next ).getEdges() );
      refuge.setBedCapacity( scenario.getRefugeBedCapacity().get( next ) );
      refuge
          .setRefillCapacity( scenario.getRefugeRefillCapacity().get( next ) );
  ...
  private void createToolActions( JMenu menu, JToolBar toolbar ) {
    ...
    addTool( new PlaceFireTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
    addTool( new RemoveFireTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
    addTool( new PlaceRefugeTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
    addTool( new RemoveRefugeTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
    addTool( new PlaceGasStationTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
    addTool( new RemoveGasStationTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
    addTool( new PlaceHydrantTool( this ), menu, toolbar, menuGroup,
        toolbarGroup );
  ...
  private void updateFireOverlay() {
    ...
    for ( int next : scenario.getFires() ) {
      fireOverlay.setBuildingDecorator( fireDecorator,
