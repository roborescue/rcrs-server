...
public class KernelControlPanel extends JPanel {
  ...
  private void addComponent(Component[] options, String type) {
    ...
    Component c = (Component) JOptionPane.showInputDialog(this, "Choose a " + type, "Choose a " + type,
  ...
  private void stepButtonPressed() {
    // This method should only be called on the event dispatch thread so it's OK to
    // update the GUI.
    // Do a sanity check just in case.
    if (!SwingUtilities.isEventDispatchThread()) {
      throw new RuntimeException(
          "stepButtonPressed called by thread " + Thread.currentThread() + ", not the event dispatch thread.");
    }
  ...
  private void runButtonPressed() {
    ...
    if (!SwingUtilities.isEventDispatchThread()) {
      throw new RuntimeException(
          "runButtonPressed called by thread " + Thread.currentThread() + ", not the event dispatch thread.");
    }
    synchronized (runLock) {
      if (running) {
        setControlButtonsEnabled(true);
