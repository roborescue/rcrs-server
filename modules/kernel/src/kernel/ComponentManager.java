...
public class ComponentManager implements ConnectionManagerListener, GUIComponent {
   ...
   * @param visibleOnStartup The set of entities that the agent should be sent on
   *                         startup. If this is null then all entities will be
   *                         sent.
   * @param agentConfig      A view of the system configuration that should be
  ...
  private class ComponentConnectionListener implements ConnectionListener {
    ...
    private void handleAKConnect(AKConnect connect, Connection connection) {
      ...
      synchronized (agentLock) {
        ...
        if (result == null) {
          ...
          Logger.info("Agent '" + connect.getAgentName() + "' id " + entity.getID() + " (" + connection + " request ID "
              + requestID + ") connected");
          // Send an OK
    ...
    private void handleSKConnect(SKConnect msg, Connection connection) throws UncompatibleScenarioException {
      int simID = getNextSimulatorID();
      int requestID = msg.getRequestID();
      Logger.info("Simulator '" + msg.getSimulatorName() + "' id " + simID + " (" + connection + " request ID "
          + requestID + ") connected");
    ...
    /**
     * Used to send info other than world model's to simulators. Information such as
     * Aftershocks' properties and etc.
     *
     * @param sim
     * @throws UncompatibleScenarioException
     */
    private void sendAdditionalInfoToSim(SimulatorProxy sim) throws UncompatibleScenarioException {
      sim.send(Collections.singleton(new KSAfterShockInfo(scenario)));
    }
    ...
    private void handleVKConnect(VKConnect msg, Connection connection) {
      ...
      Logger.info("Viewer '" + msg.getViewerName() + "' id " + viewerID + " (" + connection + " request ID " + requestID
          + ") connected");
  ...
  private static class AgentAck {
    ...
    public String toString() {
      return agent.getName() + ": " + agent.getControlledEntity().getURN() + " " + agent.getControlledEntity().getID()
