...
public class ClearSimulator extends StandardSimulator {
  ...
  private static final double REPAIR_COST_FACTOR  = 0000001;
  ...
  private void processClearArea( AKClearArea clear, ChangeSet changes ) {
    ...
    Area agentPosition = (Area) agent.getPosition( model );
    ...
    java.awt.geom.Area area = Geometry.getClearArea( agent, targetX, targetY,
        length, clearRad );
    for ( Map.Entry<Blockade, java.awt.geom.Area> entry : blockades
        .entrySet() ) {
  ...
  private boolean isValid( AKClear clear, Set<EntityID> cleared ) {
    ...
    if ( agent == null ) {
      Logger.info(
          "Rejecting clear command " + clear + ": agent does not exist" );
      return false;
    }
    ...
    if ( target == null ) {
      Logger.info(
          "Rejecting clear command " + clear + ": target does not exist" );
      return false;
    }
    ...
    if ( !( target instanceof Blockade ) ) {
      Logger.info(
          "Rejecting clear command " + clear + ": target is not a road" );
      return false;
    }
