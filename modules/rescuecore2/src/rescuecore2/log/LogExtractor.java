...
public class LogExtractor {
  ...
  public void showTimestep( int time ) {
    try {
      ...
      /*
       * for (ViewComponent next : viewers) { next.view(model, commandsRecord ==
       * null ? null : commandsRecord.getCommands(), updatesRecord == null ?
       * null : updatesRecord.getChangeSet()); next.repaint(); }
       */
      current_time = time;
  ...
  public BufferedImage paintImage() {
    ...
    BufferedImage image = configuration.createCompatibleImage( view.getWidth(),
        view.getHeight(), Transparency.TRANSLUCENT );
   ...
   *
   * @param args
   *          Command line arguments. Accepts only one argument: the name of a
   *          log file.
   */
  public static void main( String[] args ) {
    ...
    try {
      ...
      while ( log.step() ) {
        ...
        if ( log.getTime() % 50 == 0 ) {
          log.writeImage( outdir + "/snapshot-" + log.getTime() + ".png" );
        }
        scores.add( log.getScore() );
      }
