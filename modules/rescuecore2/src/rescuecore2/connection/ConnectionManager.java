...
public class ConnectionManager {
   ...
   * @param registry
   *          The registry to install in new connections.
   * @param listener
   *          A ConnectionManagerListener that will be informed of new
   *          connections.
   * @throws IOException
   *           If there is a problem listening on the port.
   */
  public void listen( int port, Registry registry,
      ConnectionManagerListener listener ) throws IOException {
    synchronized ( lock ) {
      if ( shutdown ) {
  ...
  public void shutdown() {
    ...
    for ( Reader next : readers ) {
      try {
        ...
        Logger.error(
            "ConnectionManager interrupted while shutting down read threads",
            e );
      }
    }
  }
