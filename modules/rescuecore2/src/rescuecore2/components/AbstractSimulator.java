 ...
 *
 * @param <T>
 *          The subclass of WorldModel that this simulator understands.
 */
public abstract class AbstractSimulator<T extends WorldModel<? extends Entity>>
    extends AbstractComponent<T> implements Simulator {
  ...
  @Override
  public void postConnect( Connection c, int simID, Collection<Entity> entities,
      Config kernelConfig ) {
    this.simID = simID;
  ...
  public void connect( Connection connection, RequestIDGenerator generator,
      Config config ) throws ConnectionException, ComponentConnectionException,
      InterruptedException {
    this.config = config;
    int requestID = generator.generateRequestID();
    SKConnect connect = new SKConnect( requestID, 1, getName() );
    CountDownLatch latch = new CountDownLatch( 1 );
    SimulatorConnectionListener l = new SimulatorConnectionListener( requestID,
        latch );
  ...
  protected void handleUpdate( KSUpdate u ) {
    ChangeSet changes = u.getChanges();
    int time = u.getTime();
    if ( time != lastUpdateTime + 1 ) {
      Logger
          .warn( "Received an unexpected update from the kernel. Last update: "
              + lastUpdateTime + ", this update: " + time );
    }
    lastUpdateTime = time;
    model.merge( changes );
  }
  ...
  /**
   * Handle a KSCommands object from the server. The default implementation
   * tells the kernel that nothing has changed.
   *
   * @param c
   *          The Commands object.
   */
  protected void handleCommands( KSCommands c ) {
    ...
    send( new SKUpdate( this.simID, c.getTime(), changes ) );
  }
  ...
  protected void processMessage( Message msg ) {
    if ( msg instanceof KSUpdate ) {
      ...
      if ( u.getSimulatorID() == this.simID ) {
        handleUpdate( u );
      }
    } else if ( msg instanceof KSCommands ) {
      KSCommands commands = (KSCommands) msg;
      if ( commands.getSimulatorID() == this.simID ) {
  ...
  protected boolean processImmediately( Message msg ) {
    if ( msg instanceof EntityIDResponse ) {
      ...
      if ( resp.getSimulatorID() == this.simID ) {
        synchronized ( idRequests ) {
          Logger.debug( "ID response: " + resp.getRequestID() + ", "
              + resp.getEntityIDs() );
