...
public final class EncodingTools {
	...
	public static void writeEntity(Entity e, OutputStream out)
		...
		// Type URN
		writeString(Registry.getCurrentRegistry().toURN_V1(e.getURN()), out);
		// EntityID
	...
	public static Entity readEntity(InputStream in) throws IOException {
		...
		Entity result = Registry.getCurrentRegistry().createEntity(Registry.getCurrentRegistry().toURN_V2(urn),
	...
	public static void writeProperty(Property p, OutputStream out)
			throws IOException {
		// Type
		writeString(Registry.getCurrentRegistry().toURN_V1(p.getURN()), out);
	...
	public static Property readProperty(InputStream in) throws IOException {
		...
		Property result = Registry.getCurrentRegistry().createProperty(Registry.getCurrentRegistry().toURN_V2(urn));
		if (defined) {
			int size = readInt32(in);
			byte[] content = readBytes(size, in);
			if (result != null) {
	...
	public static void writeMessage(Message m, OutputStream out)
		...
		// Type URN
		writeString(Registry.getCurrentRegistry().toURN_V1(m.getURN()), out);
		// Size
	...
	public static Message readMessage(InputStream in) throws IOException {
		...
		int size = readInt32(in);
		byte[] content = readBytes(size, in);
		Message result = Registry.getCurrentRegistry().createMessage(Registry.getCurrentRegistry().toURN_V2(urn),
