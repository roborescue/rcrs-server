...
public final class EncodingTools {
    ...
    while ( count < length ) {
      ...
      if ( read == -1 ) {
        throw new EOFException(
            "Broken input pipe. Read " + count + " bytes of " + length + "." );
      }
      count += read;
    }
   ...
   */
  public static byte[] readBytes( int size, InputStream in )
      throws IOException {
    ...
    while ( total < size ) {
      ...
      if ( read == -1 ) {
        throw new EOFException(
            "Broken input pipe. Read " + total + " bytes of " + size + "." );
      }
      total += read;
    }
    return buffer;
   ...
   *           If the DataOutput blows up.
   */
  public static void writeDouble( double d, DataOutput out )
      throws IOException {
    out.writeDouble( d );
  }
  ...
  public static double readDouble( InputStream in ) throws IOException {
    ...
    for ( int i = 0; i < data.length; ++i ) {
      ...
      if ( data[i] == -1 ) {
        throw new EOFException(
            "Broken input pipe. Read " + i + " bytes of 8." );
      }
    }
    long result = data[0] << 56 | data[1] << 48 | data[2] << 40 | data[3] << 32
  ...
  public static double readDouble( byte[] in, int offset ) {
    ...
    long result = parts[0] << 56 | parts[1] << 48 | parts[2] << 40
        | parts[3] << 32 | parts[4] << 24 | parts[5] << 16 | parts[6] << 8
        | parts[7];
    return Double.longBitsToDouble( result );
  }
   ...
   *           If the DataOutput blows up.
   */
  public static void writeFloat32( float f, DataOutput out )
      throws IOException {
    /* Aftershock Requirement:2013 */
    // DataOutput writes big-endian
