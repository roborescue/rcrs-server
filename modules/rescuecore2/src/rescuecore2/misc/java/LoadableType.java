...
public class LoadableType {
  ...
  public static final LoadableType MESSAGE_FACTORY  = new LoadableType(
      "MessageFactory", "(.+MessageFactory).class", MessageFactory.class );
  /** A CommandFactory loadable type. */
  public static final LoadableType COMMAND_FACTORY  = new LoadableType(
      "CommandFactory", "(.+CommandFactory).class", CommandFactory.class );
  /** An EntityFactory loadable type. */
  public static final LoadableType ENTITY_FACTORY   = new LoadableType(
      "EntityFactory", "(.+EntityFactory).class", EntityFactory.class );
  /** A PropertyFactory loadable type. */
  public static final LoadableType PROPERTY_FACTORY = new LoadableType(
      "PropertyFactory", "(.+PropertyFactory).class", PropertyFactory.class );
  /** An Agent loadable type. */
  public static final LoadableType AGENT            = new LoadableType( "Agent",
      "(.+(?:FireBrigade|PoliceForce|AmbulanceTeam|Centre|Center|Civilian)).class",
      Agent.class );
  /** A Simulator loadable type. */
  public static final LoadableType SIMULATOR        = new LoadableType(
      "Simulator", "(.+Simulator).class", Simulator.class );
  /** A Viewer loadable type. */
  public static final LoadableType VIEWER           = new LoadableType(
      "Viewer", "(.+Viewer).class", Viewer.class );
  /** A Component loadable type. */
  public static final LoadableType COMPONENT        = new LoadableType(
   ...
   *
   * @param manifestKey
   *          The key to look for in a jar manifest for this type.
   * @param regex
   *          A regex to use for determining if a class name should be tested.
   * @param clazz
   *          A superclass for checking if candidate classes should be
   *          extracted.
   */
      ...
      for ( String next : value.split( " " ) ) {
        try {
          ...
          if ( !clazz.isAssignableFrom( testClass ) ) {
            Logger.warn( "Manifest entry '" + manifestKey
                + "' contains invalid class name: '" + next
          ...
          Logger.warn( "Manifest entry '" + manifestKey
              + "' contains invalid class name: '" + next + "' not found" );
        }
      }
    }
    return result;
