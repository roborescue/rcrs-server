...
public final class JavaTools {
  ...
  /**
   * Instantiate a class by reflection. This method will not throw exceptions
   * but will return null on error.
   *
   * @param className
   *          The class name to instantiate.
   * @param outputClass
   *          The class that specifies the type that should be returned. This
   *          will usually be a superclass of the given class name.
   * @param <T>
   ...
   * doesn't exist (or in inaccessible or the wrong type) then a constructor
   * will be used.
   *
   * @param className
   *          The class name to instantiate.
   ...
   * @param <T>
   *          The desired return type.
   * @return The content of the INSTANCE field if it exists; a new instance of
   *         the given class name cast as the output class, or null if the class
   *         cannot be instantiated.
   */
  public static <T> T instantiateFactory( String className,
      Class<T> outputClass ) {
    ...
    try {
      ...
      Logger.info( className + " is not a subclass of " + outputClass.getName(),
          e );
      return null;
    }
    ...
    try {
      ...
      if ( Modifier.isStatic( field.getModifiers() ) ) {
        try {
          ...
          Logger.info( "Could not access INSTANCE field in class " + className
              + ": trying constructor." );
        } catch ( ClassCastException e ) {
          Logger.info( "Could not cast INSTANCE field to " + outputClass
              + " in class " + className + ": trying constructor." );
        }
      }
