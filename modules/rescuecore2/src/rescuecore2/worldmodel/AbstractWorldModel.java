...
public abstract class AbstractWorldModel<T extends Entity>
  ...
  public final void addEntity( Entity e ) {
    if ( isAllowed( e ) ) {
      ...
      throw new IllegalArgumentException( this
          + " does not accept entities of type " + e.getClass().getName() );
    }
  }
  ...
  public void merge( ChangeSet changeSet ) {
    for ( EntityID e : changeSet.getChangedEntities() ) {
      ...
      if ( existingEntity == null ) {
        ...
        existingEntity = Registry.getCurrentRegistry()
            .createEntity( changeSet.getEntityURN( e ), e );
        if ( existingEntity == null ) {
          // Bail out
          continue;
        }
   ...
   * default implementation checks that the object is assignable to at least one
   * class previously registered with {@link #registerAllowedClass(Class)}.
   *
   * @param e
   *          The entity to check.
