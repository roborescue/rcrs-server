  ...
  protected void pushImpl(AKSpeak speak, int originalSize) throws InvalidMessageException {
    ...
    if (!(e instanceof Human)) {
      throw new InvalidMessageException(
          "Agent " + agentID + " is not a human: " + (e == null ? "null" : e.getClass().getName()));
    }
    ...
    if (originalSize > maxSize) {
      throw new InvalidMessageException("Agent " + agentID + " tried to send an oversize voice message: " + data.length
          + " bytes but the limit is " + maxSize);
    }
    uttered.put(agentID, count + 1);
    // Find out who can hear it
    StandardEntity sender = world.getEntity(agentID);
    for (StandardEntity target : world.getEntitiesOfType(StandardEntityURN.FIRE_BRIGADE,
        StandardEntityURN.AMBULANCE_TEAM, StandardEntityURN.POLICE_FORCE, StandardEntityURN.CIVILIAN)) {
