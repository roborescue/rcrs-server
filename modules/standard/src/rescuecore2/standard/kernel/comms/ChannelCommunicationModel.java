...
public class ChannelCommunicationModel extends AbstractCommunicationModel {
  ...
  public void process(int time, Collection<? extends Command> agentCommands) {
    ...
    for (Entity agent : world.getEntitiesOfType(StandardEntityURN.FIRE_BRIGADE, StandardEntityURN.FIRE_STATION,
        StandardEntityURN.POLICE_FORCE, StandardEntityURN.POLICE_OFFICE, StandardEntityURN.AMBULANCE_TEAM,
  ...
  private void processSubscribe(AKSubscribe sub) {
    ...
    if (requested.size() > max) {
      Logger.warn(
          "Agent " + id + " tried to subscribe to " + requested.size() + " channels but only " + max + " allowed");
      return;
    }
