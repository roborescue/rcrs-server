...
public class DropoutNoise implements Noise {
  ...
  /**
   * Construct a DropoutNoise object that will wipe out messages with some
   * probability.
   *
   * @param p
   *          The probability of destroying a message.
   * @param random
   *          The RNG to use.
   */
  ...
  public AKSpeak applyNoise( AKSpeak message ) {
    ...
    return new AKSpeak( message.getAgentID(), message.getTime(),
        message.getChannel(), new byte[0] );
  }
}
