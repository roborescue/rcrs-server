...
public class RadioChannel extends AbstractChannel {
  ...
  public RadioChannel( Config config, int channelID ) {
    super( channelID );
    bandwidth = config.getIntValue(
        ChannelCommunicationModel.PREFIX + channelID + BANDWIDTH_SUFFIX );
  }
  ...
  @Override
  public void pushImpl( AKSpeak speak, int originalSize )
    ...
    if ( usedBandwidth > bandwidth ) {
      throw new InvalidMessageException( "Discarding message on channel "
          + channelID + ": already used " + usedBandwidth + " of " + bandwidth
          + " bytes, new message is " + data.length + " bytes." );
    }
