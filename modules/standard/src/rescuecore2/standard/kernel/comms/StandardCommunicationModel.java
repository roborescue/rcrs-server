...
public class StandardCommunicationModel extends AbstractCommunicationModel {
  ...
  private void processSay(AKSay say) throws InvalidMessageException {
    ...
    if (!(sender instanceof Human)) {
      throw new InvalidMessageException(
          "Agent " + senderID + " is not a human: " + (sender == null ? "null" : sender.getClass().getName()));
    }
    byte[] data = say.getContent();
    if (data.length > maxSize) {
      throw new InvalidMessageException(
          "Agent " + senderID + " sent an oversize message: " + data.length + " > " + maxSize);
    }
    int count = uttered.get(senderID);
    int max = getMessageMax(sender);
    if (count >= getMessageMax(sender)) {
      throw new InvalidMessageException(
          "Agent " + senderID + " has uttered too many messages: " + count + " >= " + max);
    }
    uttered.put(senderID, count + 1);
    for (StandardEntity receiver : world.getEntitiesOfType(StandardEntityURN.CIVILIAN, StandardEntityURN.FIRE_BRIGADE,
        StandardEntityURN.FIRE_STATION, StandardEntityURN.AMBULANCE_TEAM, StandardEntityURN.AMBULANCE_CENTRE,
  ...
  private boolean canHear(StandardEntity receiver, StandardEntity sender) {
    ...
    if (receiver instanceof FireStation) {
      return sender instanceof FireBrigade || sender instanceof FireStation || sender instanceof PoliceOffice
    ...
    if (receiver instanceof PoliceOffice) {
      return sender instanceof PoliceForce || sender instanceof FireStation || sender instanceof PoliceOffice
          || sender instanceof AmbulanceCentre;
    }
    ...
    if (receiver instanceof AmbulanceCentre) {
      return sender instanceof AmbulanceTeam || sender instanceof FireStation || sender instanceof PoliceOffice
          || sender instanceof AmbulanceCentre;
    }
    return false;
  }
