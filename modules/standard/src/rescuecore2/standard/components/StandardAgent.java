...
import rescuecore2.standard.commands.AKTell;
...
public abstract class StandardAgent<E extends StandardEntity>
  ...
  protected void sendRest( int time ) {
    send( new AKCommand( new AKRest( getID(), time ) ) );
  }
  ...
  protected void sendMove( int time, List<EntityID> path ) {
    send( new AKCommand( new AKMove( getID(), time, path ) ) );
  }
   ...
   */
  protected void sendMove( int time, List<EntityID> path, int destX,
      int destY ) {
    send( new AKCommand( new AKMove( getID(), time, path, destX, destY ) ) );
  }
  ...
  protected void sendExtinguish( int time, EntityID target, int water ) {
    send( new AKCommand( new AKExtinguish( getID(), time, target, water ) ) );
  }
  ...
  protected void sendClear( int time, EntityID target ) {
    send( new AKCommand( new AKClear( getID(), time, target ) ) );
  }
   ...
   *          The current time.
   * @param destX
   *          The destination X coordinate to clear.
   * @param destY
   *          The destination Y coordinate to clear.
   */
  protected void sendClear( int time, int destX, int destY ) {
    send( new AKCommand( new AKClearArea( getID(), time, destX, destY ) ) );
  }
  ...
  protected void sendRescue( int time, EntityID target ) {
    send( new AKCommand( new AKRescue( getID(), time, target ) ) );
  }
  ...
  protected void sendLoad( int time, EntityID target ) {
    send( new AKCommand( new AKLoad( getID(), time, target ) ) );
  }
  ...
  protected void sendUnload( int time ) {
    send( new AKCommand( new AKUnload( getID(), time ) ) );
  }
  ...
  protected void sendSpeak( int time, int channel, byte[] data ) {
    send( new AKCommand( new AKSpeak( getID(), time, channel, data ) ) );
  }
  ...
  protected void sendSubscribe( int time, int... channels ) {
    send( new AKCommand( new AKSubscribe( getID(), time, channels ) ) );
  }
  ...
  protected void sendSay( int time, byte[] data ) {
    send( new AKCommand( new AKSay( getID(), time, data ) ) );
  }
  ...
  protected void sendTell( int time, byte[] data ) {
    send( new AKCommand( new AKTell( getID(), time, data ) ) );
  }
